%getROI_Mask
%This function will get the exposure time, ROI, and Mask for the IR and GFP
%cameras. The experiment should be set up and a few sample flies should be placed
%in so that proper exposure times can be chosen. A live view from the
%cameras will be then be shown. Cameras can now be focused and adjusted and
...
%think). Once satisfied, then set up a flashlight underneath the chamber so
%that the GFP camera will have a signal that shows the boundaries. then
%click done.
...
stoppreview(vid_IR);stoppreview(vid_GFP)
close(gcf)
...
fig = figure;
imshow(im_GFP,'InitialMagnification','fit');
disp('Create rectangular ROI!')
hmask = impoly ;
disp('Double click rectangle to finish!')
wait(hmask);
hmask = createMask(hmask);
[y,x] = find(hmask);
ROI_position_GFP = [min(x)-5,min(y)-5,max(x)-min(x)+10,max(y)-min(y)+10];
close(gcf)
delete(vid_GFP); %Have to restart video to set the ROIposition property.
vid_GFP=videoinput('gentl',2, 'Mono8');
vid_GFP.ReturnedColorSpace = 'grayscale';
triggerconfig(vid_GFP,'Manual');
vid_GFP.FramesPerTrigger = 1;
vid_GFP.TriggerRepeat = Inf;
vid_GFP.ROIPosition = ROI_position_GFP;
set(vid_GFP,'Timeout',50); %set the Timeout property of VIDEOINPUT object 'vid' to 50 seconds
start(vid_GFP);
trigger(vid_GFP);
[im_GFP,time] = getdata(vid_GFP,1);
figure;
...
disp('Double click polygon to finish!')
wait(hmask);
bwMask_GFP = createMask(hmask) ;

params_GFP = struct('ROI_position',ROI_position_GFP,'bwMask',bwMask_GFP,'expsr_time',expsr_time_GFP);
