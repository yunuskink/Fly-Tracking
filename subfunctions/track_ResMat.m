function [ResMat] = track_ResMat(ResMat,maxdisp)

%YUNUS:  So it's handy to run tracking separate from image analysis.
%Xiaoning thinks that the length of a fly is a good estimate for the
%maximum fly step.

[blah,order] = sort(ResMat(:,1),'Ascend');
ResMat = ResMat(order,:);
...
tracks = newtracks;
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%Must fix this for orientations later!!!!  Weird error with dimension
%mismatch at the line after the next one.
ResMat = zeros(size(tracks,1),7);
ResMat(:,[1 2 3 7]) = tracks(:,[3 1 2 4]);
ResMat = sortrows(ResMat, [1 7]);
%save('ResMat_with_track','ResMat')
