...
presgrad=-logligrad(rootparams ,  MaxPop , Nbins, Tframes, hist) %%steepest descent
alpha=linemin(rootparams,presgrad, alpharoot,  MaxPop, Nbins,Tframes,hist); %%finding an adecuate step size
...
tolerance=0.001; %%when the gradient's magnitude is smaller than this, the search stops
%counter=0;

while sqrt(logligrad(params ,MaxPop,Nbins,Tframes, hist)'*logligrad(params ,MaxPop,Nbins,Tframes, hist))>tolerance 
   conjugdir=getconjugdir(params,pastparams,pastconjugdir, MaxPop,Nbins,Tframes,hist);  %%preconditioner used is PR, also it can get automatic resets to steepest descent
   ...
   alpha=linemin(params,conjugdir,alpharoot,  MaxPop,Nbins,Tframes, hist) %%finding an adecuate step size
   ...
   sqrt(logligrad(params ,  MaxPop,Nbins,Tframes, hist)'*logligrad(params ,  MaxPop,Nbins,Tframes, hist)) %%checking the norm of the gradient
   %counter=counter+1
end
